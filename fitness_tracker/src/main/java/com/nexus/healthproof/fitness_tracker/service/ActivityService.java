...
public class ActivityService {
    ...
    public List<Activity> read(UUID userId, String name) {
        ...
        if (activities.isEmpty()) {
            throw new ActivityNotFoundException("No activities found with name: " + name + " for user: " + userId);
        }
        return activities;
    }
    ...
    public Activity update(UUID userId, UUID id, Activity activity) {
        ...
        return activityRepository.findById(id)
                ...
                .map(act -> {
                    if (activity.getName() != null) act.setName(activity.getName());
                    if (activity.getDuration() != null) act.setDuration(activity.getDuration());
                    if (activity.getDistance() != null) act.setDistance(activity.getDistance());
                    if (activity.getCaloriesBurned() != null) act.setCaloriesBurned(activity.getCaloriesBurned());
                    if (activity.getStartDate() != null) act.setStartDate(activity.getStartDate());
                    if (activity.getEndDate() != null) act.setEndDate(activity.getEndDate());
                    if (activity.getIntensityLevel() != null) act.setIntensityLevel(activity.getIntensityLevel());
    ...
    public void deleteAll(UUID userId) {
        ...
        if (activities.isEmpty()) {
            throw new ActivityNotFoundException("No activities to delete for user: " + userId);
        }
    ...
    public Boolean delete(UUID userId, UUID id) {
        ...
        return activityRepository.findById(id)
                .filter(act -> act.getUser().equals(user))
                .map(act -> { activityRepository.delete(act); return true; })
                .orElseThrow(() -> new ActivityNotFoundException("Activity not found with id: " + id));
    }
}
